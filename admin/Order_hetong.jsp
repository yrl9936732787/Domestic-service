...
function updateOrder(){
	...
	var money = $("#money").val();
	var aunt_signature = $("#aunt_signature").val();
	var aunt_idcard = $("#aunt_idcard").val();
	var aunt_phone = $("#aunt_phone").val();
	var aunt_signdate = $("#aunt_signdate").val();
	
	$.ajax({
		url : "${pageContext.request.contextPath }/Update_creatOrder.action",
		type : "post", 
		// data表示发送的数据
		data :JSON.stringify({
			oid:oid,
		...
		success : function(data){
			if(data){
				 var msg = "提交乙方合约信息成功！请等待甲方签订";
			        if (confirm(msg)==true){
			        	 window.location.href="${pageContext.request.contextPath }/toOrder.action"
					}
...
<script>	
	function datetime() {
		 var now = new Date();
		 document.getElementById("aunt_signdate").value = now.getFullYear() + "-"
...
<script type="text/javascript">
function a(){
	var aunt_signature= $("#aunt_signature");
	if(aunt_signature.val()==""){alert("签名不可为空");}

}
function b(){
	var money= $("#money");
	if(money.val()==""){alert("价格不可为空");}

}
function c(){
	var aunt_idcard= $("#aunt_idcard");
	if(aunt_idcard.val()==""){alert("身份证不可为空");}

}
function d(){
	var aunt_phone= $("#aunt_phone");
	if(aunt_phone.val()==""){alert("签名不可为空");}
...
<table width="80%" align="center">
	...
	<tr>
		<td colspan="2">
			<p><h3>甲方姓名（雇主）：</h3></p>
			<p><h3>乙方姓名（雇员）：<input type="text" id="aunt_name" name="aunt_name" value="${order.aunt_name}" required /></h3></p>
		</td>
	</tr>
 	<tr>
		<td colspan="2">
			...
			<p><h4>&emsp;&emsp;二、 由甲、乙双方商定以下乙方到甲方做家政的《上岗协议》  内容：<input type="text" id="hetong_content" name="hetong_content" value="${order.hetong_content}" style="width:350px;" required />  
			乙方从<input type="text" id="dangqian_work_begin" name="dangqian_work_begin" value="${order.dangqian_work_begin}"  />起
			至<input type="text" id="dangqian_work_end" name="dangqian_work_end" value="${order.dangqian_work_end}" />，自愿到甲方家工作，
			工作地址__(客户填写)__，吃住全包，月休息俩天（不包括晚上，晚上必须在甲方家里），（注明：俩天假期必须通过双方协商后再定）,乙方可以把每个月休息日累计起来等到假期或者有事情的时候
			休息，乙方除了休息日外， 其他时间不能请假，影响雇主正常的生活。国定假日不可休息。工资<input type="text" id="money" name="money" value="${order.money}" onblur="b()" required />，每15天结清工资。 </h4></p>
	...
	<tr>
		<td ><h4>甲方签名：</h4></td>
		<td ><h4>乙方签名：<input type="text" id="aunt_signature" name="aunt_signature" value="${order.aunt_signature}" onblur="a()" required /></h4></td>
	</tr>
	 <tr>
		<td><h4>身份证号码：</h4></td>
		<td><h4>身份证号码：<input type="text" id="aunt_idcard" style="width:200px" name="aunt_idcard" value="${order.aunt_idcard}" onblur="c()" /></h4></td>
	</tr>
	<tr>
		<td><h4>联系电话：</h4></td>
		<td><h4>联系电话：<input type="text" id="aunt_phone" name="aunt_phone" value="${order.aunt_phone}" onblur="d()" /></h4></td>
	</tr>
	<tr>
		<td><h4>签订日期：</h4></td>
		<td><h4>签订日期：<input type="text"  id="aunt_signdate" name="aunt_signdate" value=""/></h4></td>
	</tr> 
	<tr>
		<td colspan="2" align="right"><button onclick="updateOrder()" class="btn btn-warning" 
